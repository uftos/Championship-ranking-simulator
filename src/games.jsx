...
export function Games() {
  ...
  var stillGame = true;
  while (stillGame) {
    const response = await fetch('http://localhost:8010/proxy/api/compets/426990/phases/1/poules/1/matchs?page='+page);
    const json = await response.json();
    if (json.hasOwnProperty("hydra:member") && (page<25)) {
      gamesInfo = gamesInfo.concat( json["hydra:member"].map((game) => {
        return {
